...
from training.utils import setup_directories, freeze_backbone, compute_classwise_alpha, plot_alpha_trends # type: ignore
...
def local_train(
    ...
    # === Initialize alpha tensor for smoothing
    ...
    # === Resume checkpoint ===
    #init_weight_path = '/Users/ticaurisstokes/Desktop/grad_research/save_dir/trial_1_seed_89701/fold_2_efficientnet_b7_cbam/weights/fold_2_efficientnet_b7_cbam_best.pth'
    ...
    #unfreezer = GradualUnfreezer(model, base_lr=lr, start_epoch=6, unfreeze_every=2, max_blocks=3) 
    ...
    unfreeze_every=2,
    max_blocks=3,
    weight_decay=1e-4  # Match your base optimizer
)
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        if epoch >= warmup_epochs:
            ...
            if val_loader:
                ...
                dynamic_alpha  = compute_classwise_alpha(y_true, y_pred, num_classes=num_classes, normalize=True, clip_range=(0.1, 3.0), prev_alpha=alpha, beta=0.9, smoothing=True)

                # === Log alpha weights per epoch
                alpha_values = dynamic_alpha.detach().cpu().numpy()
                # Optional: print alpha changes
                print(f"📊 Alpha Weights (Epoch {epoch+1}): {np.round(alpha_values, 4)}")
    ...
    #alpha_plot_path = os.path.join(graph_dir, f"{model_name}_alpha_trend.png")
    #plot_alpha_trends(alpha_history, num_classes=num_classes, save_path=alpha_plot_path)
